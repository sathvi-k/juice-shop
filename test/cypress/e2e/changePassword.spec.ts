describe('/#/privacy-security/change-password', () => {
  describe('as Morty', () => {
    beforeEach(() => {
      cy.login({
        email: Cypress.env('mortyEmail'),
        password: Cypress.env('mortyPassword')
      })
  ...
  describe('challenge "changePasswordBenderChallenge"', () => {
    it('should be able to change password via XSS-powered attack on password change without passing current password', () => {
      cy.login({
        email: Cypress.env('benderEmail'),
        password: Cypress.env('benderPassword')
      })
      cy.visit(
      ...
      cy.wait(2000)
      cy.login({ email: Cypress.env('benderEmail'), password: Cypress.env('benderNewPassword') })

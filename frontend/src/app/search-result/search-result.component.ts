...
export class SearchResultComponent implements OnDestroy, AfterViewInit {
  ...
  filterTable () {
    ...
    if (queryParam) {
      ...
      const escapedQueryParam = queryParam.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;').replace(/'/g, '&#39;')
      this.searchValue = this.sanitizer.sanitize(1, escapedQueryParam) // vuln-code-snippet vuln-line localXssChallenge xssBonusChallenge

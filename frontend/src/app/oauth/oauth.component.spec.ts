...
describe('OAuthComponent', () => {
  ...
  it('will create regular user account with base64 encoded reversed email as password', fakeAsync(() => {
    ...
    component.ngOnInit()
    expect(userService.save).toHaveBeenCalledWith({ email: 'test@test.com', password: process.env.TEST_OAUTH_PASSWORD, passwordRepeat: process.env.TEST_OAUTH_PASSWORD })
  }))
  ...
  it('logs in user even after failed account creation as account might already have existed from previous OAuth login', fakeAsync(() => {
    ...
    component.ngOnInit()
    expect(userService.login).toHaveBeenCalledWith({ email: 'test@test.com', password: process.env.TEST_OAUTH_PASSWORD, oauth: true })
  }))

...
export function performRedirect () {
  return ({ query }: Request, res: Response, next: NextFunction) => {
    ...
    const allowedList: string[] = security.redirectAllowlist instanceof Set ? [...security.redirectAllowlist] : []
    let matchedIndex = -1
    for (let i = 0; i < allowedList.length; i++) {
      if (utils.startsWith(toUrl, allowedList[i])) {
        matchedIndex = i
        break
      }
    }
    if (security.isRedirectAllowed(toUrl) && matchedIndex >= 0) {
      ...
      res.redirect(allowedList[matchedIndex])
    } else {
      res.status(406)
